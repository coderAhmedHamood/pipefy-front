...
class PermissionService {
  ...
  // إلغاء صلاحية من مستخدم بناءً على المرحلة (stage_id)
  static async revokeUserPermissionByStage(userId, processId, stageId) {
    try {
      ...
      // التحقق من وجود السجل قبل الحذف
      const beforeDeleteCheck = await pool.query(
        `SELECT id, user_id, permission_id, stage_id, process_id, granted_at 
         FROM user_permissions 
         WHERE user_id = $1 AND stage_id = $2 AND process_id = $3`,
        [userId, stageId, processId]
      );
      
      if (beforeDeleteCheck.rows.length === 0) {
        // محاولة البحث عن السجلات المشابهة للمساعدة في التشخيص
        const similarRecords = await pool.query(
          `SELECT id, user_id, stage_id, process_id, granted_at 
           FROM user_permissions 
        ...
        if (similarRecords.rows.length > 0) {
          const processIds = similarRecords.rows.map(r => r.process_id).join(', ');
          throw new Error(`المرحلة غير مرتبطة بالمستخدم في هذه العملية. المرحلة موجودة في العمليات: ${processIds}`);
        } else {
          throw new Error('المرحلة غير مرتبطة بالمستخدم في هذه العملية');
        }
      }
      
      // حذف السجل بناءً على user_id, stage_id, process_id
      const deleteQuery = `
        DELETE FROM user_permissions 
        WHERE user_id = $1 
          AND stage_id = $2
          AND process_id = $3
        RETURNING id, user_id, permission_id, stage_id, process_id, granted_at
      `;

      const { rows } = await pool.query(deleteQuery, [userId, stageId, processId]);
      
      if (rows.length === 0) {
        throw new Error('فشل حذف السجل - لم يتم العثور على السجل');
      }
      ...
      if (afterDeleteCheck.rows.length > 0) {
        throw new Error('فشل حذف السجل - السجل ما زال موجوداً بعد الحذف');
      }

      return { 
        message: 'تم إلغاء الصلاحية من المستخدم في المرحلة بنجاح',
        deleted_count: rows.length,
        deleted_record: rows[0],
        user: userCheck.rows[0],
