...
1. ✅ السيرفر يعمل على `http://localhost:3004`
...
```bash
curl -X DELETE "http://localhost:3004/api/ticket-reviewers/REVIEWER_ID_HERE" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```
...
```bash
curl -X GET "http://localhost:3004/api/tickets/7a6981d3-5683-46cf-9ca1-d1f06bf8a154/reviewers" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```
...
```bash
curl -X POST http://localhost:3004/api/ticket-assignments \
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  -H "Content-Type: application/json" \
  -d '{
    "ticket_id": "7a6981d3-5683-46cf-9ca1-d1f06bf8a154",
...
```bash
curl -X DELETE "http://localhost:3004/api/ticket-assignments/ASSIGNMENT_ID_HERE" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```
...
```bash
curl -X DELETE "http://localhost:3004/api/ticket-reviewers/REVIEWER_ID_HERE?hard=true" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```
...
```bash
curl -X DELETE "http://localhost:3004/api/ticket-assignments/ASSIGNMENT_ID_HERE?hard=true" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```
