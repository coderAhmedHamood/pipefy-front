...
async function testGetUserAdditionalPermissions() {
  try {
    ...
    console.log(`📋 جلب الصلاحيات الإضافية للمستخدم: ${USER_ID}`);
    console.log(`📋 في العملية: ${PROCESS_ID}\n`);
    
    const response = await axios.get(
      `${BASE_URL}/api/permissions/users/${USER_ID}?process_id=${PROCESS_ID}`,
      {
        headers: {
    ...
    if (response.data.success) {
      const { active_permissions, inactive_permissions, stats, user, process } = response.data.data;
      ...
      if (active_permissions.length > 0) {
        ...
        active_permissions.forEach((perm, index) => {
          ...
          if (perm.granted_by_name) {
            console.log(`      - منحت بواسطة: ${perm.granted_by_name}`);
          }
          if (perm.expires_at) {
...
testGetUserAdditionalPermissions();
