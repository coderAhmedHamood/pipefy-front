...
const UsersList = () => {
  ...
  const toggleUserStatus = async (userId) => {
    ...
    try {
      await axios.patch(`http://localhost:3000/api/users/${userId}/toggle-status`, {}, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
...
const roleManager = {
  ...
  async updateRolePermissions(roleId, permissions) {
    const token = localStorage.getItem('token');
    const response = await axios.put(`http://localhost:3000/api/roles/${roleId}/permissions`, {
      permissions
    }, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
...
const getDashboardStats = async () => {
  ...
  try {
    const [userStats, permissionStats] = await Promise.all([
      ...
      axios.get('http://localhost:3000/api/permissions/stats', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
    ]);
