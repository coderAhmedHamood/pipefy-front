...
// إنشاء إشعار مباشرة
async function createNotificationDirect(notificationData) {
  try {
    const result = await pool.query(`
    ...
    // إرسال الإيميل
    await NotificationController.sendNotificationEmail({
      userIds: [notificationData.user_id],
      title: notificationData.title,
      message: notificationData.message,
      notificationType: notificationData.notification_type,
      actionUrl: notificationData.action_url || notificationData.url,
...
async function main() {
  try {
    ...
    const notifications = [
      {
        ...
        action_url: '/tickets',
        data: { ticket_id: 'test-123', test: true }
      },
      {
        user_id: TEST_USER_ID,
      ...
      },
      {
        ...
        action_url: '/tickets',
        data: { ticket_id: 'test-123', comment_id: 'comment-123', test: true }
      },
      {
        user_id: TEST_USER_ID,
        ...
        action_url: '/tickets',
        data: { ticket_id: 'test-123', from_stage: 'المرحلة السابقة', to_stage: 'المرحلة التالية', test: true }
      },
      {
        user_id: TEST_USER_ID,
        ...
        action_url: '/tickets',
        data: { ticket_id: 'test-123', review_id: 'review-123', test: true }
      },
      {
        user_id: TEST_USER_ID,
      ...
      },
      {
        ...
        action_url: '/tickets',
        data: { ticket_id: 'test-123', due_date: new Date().toISOString(), test: true }
      },
      {
        user_id: TEST_USER_ID,
        ...
        action_url: '/tickets',
        data: { ticket_id: 'test-123', review_status: 'completed', test: true }
      }
    ];
    ...
    for (let i = 0; i < notifications.length; i++) {
      ...
      try {
        const created = await createNotificationDirect(notification);
        log(`✅ تم إنشاء الإشعار بنجاح`, 'green');
        log(`   ID: ${created.id}`, 'cyan');
        log(`   📧 تم إرسال الإيميل`, 'cyan');
        results.push({ type: notification.notification_type, success: true });
      } catch (error) {
        log(`❌ فشل: ${error.message}`, 'red');
        results.push({ type: notification.notification_type, success: false, error: error.message });
      }
    ...
    results.forEach((r, i) => {
      if (r.success) {
        log(`✅ ${i + 1}. ${r.type} - تم الإنشاء والإرسال`, 'green');
      } else {
        log(`❌ ${i + 1}. ${r.type} - فشل`, 'red');
        if (r.error) log(`   الخطأ: ${r.error}`, 'red');
      }
    });
